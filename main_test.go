...
func TestOldToNewRoundtrip(t *testing.T) {
	out := new(bytes.Buffer)
	err := convert(strings.NewReader(oldConfigNoPath), out, ver5to6)
	if err != nil {
		t.Fatal(err)
	}
	convConfig := out.String()
	out = new(bytes.Buffer)
	err = convert(strings.NewReader(convConfig), out, ver6to5)
	if err != nil {
		t.Fatal(err)
	}
...
func TestNewToOldRoundtrip(t *testing.T) {
	out := new(bytes.Buffer)
	err := convert(strings.NewReader(newConfig), out, ver6to5)
	if err != nil {
		t.Fatal(err)
	}
	convConfig := out.String()
	out = new(bytes.Buffer)
	err = convert(strings.NewReader(convConfig), out, ver5to6)
	if err != nil {
		t.Fatal(err)
	}
	revertedConfig := out.String()
